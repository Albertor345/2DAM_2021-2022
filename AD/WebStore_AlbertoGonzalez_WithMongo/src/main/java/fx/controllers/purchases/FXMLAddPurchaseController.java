...
public class FXMLAddPurchaseController implements Initializable {
    ...
    private void addPurchase() {
        ...
        if (item != null && customer != null) {
            Purchase purchase = Purchase.builder()
                    ...
                    .itemID(item.get_id().toString())
                    .itemName(item.getName())
                    .date(dateBox.getValue()).build();

            if (principalController.getServicesItems().addPurchase(purchase)) {
    ...
    public void load(List<Item> items, List<Customer> customers) {
        if (principalController.isAdmin()) {
            ...
            loadCustomersList(customers.stream()
                    .filter(customer -> customer.get_id().equals(principalController.getUser().get_id()))
